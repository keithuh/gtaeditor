...
func _load_image():
	...
	# DXT COMPRESSION: this is only for gta san andreas, i don't know if gta vc needs this
	# verify DXT compression (only for 8 and 9)
	if compression == 8:  # DXT1
		return _load_dxt1()
	elif compression == 9:  # DXT3
		return _load_dxt3()
